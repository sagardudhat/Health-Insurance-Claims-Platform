...
interface ReviewerAllClaimsFilterBarProps {
  searchParam: string;
  statusParam: string;
...
export const ReviewerAllClaimsFilterBar: React.FC<ReviewerAllClaimsFilterBarProps> = ({
  searchParam,
  statusParam,
  searchFieldParam,
  flaggedOnlyParam,
  ...
  const [localSearchField, setLocalSearchField] = useState(searchFieldParam);

  useEffect(() => {
    setLocalSearchInput(searchParam);
    setLocalSearchField(searchFieldParam);
  }, [searchParam, searchFieldParam]);

  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setSearchInput(localSearchInput);
    setSelectedSearchField(localSearchField);
    updateQueryParams({
